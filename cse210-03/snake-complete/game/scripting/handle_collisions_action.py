...
class HandleCollisionsAction(Action):
    ...
    def _handle_segment_collision(self, cast):
        ...
        segments_one_head = cycle_one.get_segments()[0]
        segments_two_head = cycle_two.get_segments()[0]

        segments_one_tail = cycle_one.get_segments()[-1]
        segments_two_tail = cycle_two.get_segments()[-1]
        
        # Finds which user wins and displays their name

        # If cycle_two hits cycle_one's tail then displays cycle_one wins
        if segments_two_head.get_position().equals(segments_one_tail.get_position()):
            score2.reduce_points()
            self._game_over_message = f"{cycle_one.get_name()} wins!"
            self._is_game_over = True
        
        # If cycle_one hits cycle_one's head hits it's own tail cycle_two wins
        if segments_one_head.get_position().equals(segments_one_tail.get_position()):
        ...
        if segments_one_head.get_position().equals(segments_two_tail.get_position()):
            score1.reduce_points()
            self._game_over_message = f"{cycle_two.get_name()} wins!"
            self._is_game_over = True

        if segments_two_head.get_position().equals(segments_two_tail.get_position()):
            score2.reduce_points()
            self._game_over_message = f"{cycle_one.get_name()} wins!"
            self._is_game_over = True
